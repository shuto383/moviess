...
function createDefaultState() {
  return {
    ...
    textOverlays: [] // {text, start, end, size, color, pos}
  };
}
...
function textOptionsHTML() {
  ...
  const banner = editing
    ? `<div class="edit-mode-banner"><span>テキストを編集中（タイムラインのマーカーから選択）</span><button id="cancelTextEdit">キャンセル</button></div>`
    : (pendingRange
  ...
  return `
    ...
    <div class="field-row">
      <div class="field">
        <label>サイズ <span class="value-pill" id="textSizeVal">${v.size}</span></label>
        <input type="range" id="newTextSize" min="16" max="120" value="${v.size}">
      </div>
      <div class="field">
    ...
    <div class="field">
      <label>位置（縦方向）</label>
      <select id="newTextPos">
        <option value="top" ${v.pos === "top" ? "selected" : ""}>上</option>
        <option value="center" ${v.pos === "center" ? "selected" : ""}>中央</option>
        <option value="bottom" ${v.pos === "bottom" ? "selected" : ""}>下</option>
      </select>
    </div>
    ${editing
      ? `<div class="btn-row">
    ...
    <p class="hint">秒は小数(0.1秒単位)で指定できます。「現在位置」ボタンで再生中の位置をそのまま入力できます。タイムラインの緑色のマーカーをクリックすると、そのテキストを選んで編集・削除できます。</p>
  `;
}
...
function bindTextEvents() {
  ...
  document.getElementById("addTextBtn")?.addEventListener("click", () => {
    ...
    const size = parseInt(document.getElementById("newTextSize").value);
    const color = document.getElementById("newTextColor").value || "#ffffff";
    const pos = document.getElementById("newTextPos").value;
    const item = {
      text,
